...
unsigned int s5kjn5_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData);
static unsigned int s5kjn5_mot_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
...
#define S5KJN5_MOT_MNF_CHECKSUM_ADDR 0x25
#define S5KJN5_MOT_AF_ADDR 0x27
#define S5KJN5_MOT_AF_DATA_SIZE 24
#define S5KJN5_MOT_AF_CHECKSUM_ADDR 0x3F
#define S5KJN5_MOT_AWB_ADDR 0x41
#define S5KJN5_MOT_AWB_DATA_SIZE 43
#define S5KJN5_MOT_AWB_CHECKSUM_ADDR 0x6C
#define S5KJN5_MOT_LSC_ADDR 0x0C96
#define S5KJN5_MOT_LSC_DATA_SIZE 0x074C
#define S5KJN5_MOT_LSC_CHECKSUM_ADDR 0X13E2
#define S5KJN5_MOT_PDAF1_ADDR 0x13F9
#define S5KJN5_MOT_PDAF2_ADDR 0x15E9
#define S5KJN5_MOT_PDAF_DATA1_SIZE 496
#define S5KJN5_MOT_PDAF_DATA2_SIZE 1004+1100
#define S5KJN5_MOT_PDAF_DATA_SIZE (S5KJN5_MOT_PDAF_DATA1_SIZE+S5KJN5_MOT_PDAF_DATA2_SIZE)
#define S5KJN5_MOT_PDAF_CHECKSUM_ADDR 0x1E21
#define S5KJN5_MOT_AF_SYNC_DATA_ADDR 0x1E25
#define S5KJN5_MOT_AF_SYNC_DATA_SIZE 24
#define S5KJN5_MOT_AF_SYNC_DATA_CHECKSUM_ADDR 0x1E3D

#define S5KJN5_MOT_MTK_NECESSARY_DATA_ADDR 0x13E4
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x0000000B, 0x54510341, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000000, 0x00000000, 0x00000000, do_module_version},
		{0x00000000, 0x00000000, 0x00000025, do_part_number},
		{0x00000001, S5KJN5_MOT_LSC_ADDR, S5KJN5_MOT_LSC_DATA_SIZE, mot_do_single_lsc},
		{0x00000001, 0x00000027, 0x00000047, s5kjn5_mot_do_2a_gain},
		{0x00000001, 0x00000000, 0x00000000, mot_s5kjn5_do_pdaf},
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_s5kjn5_eeprom = {
	.name = "mot_s5kjn5_eeprom",
	.sensor_type = TELE_CAMERA,
	.check_layout_function = mot_layout_no_ck,
	.read_function = Common_read_region,
	.mot_do_factory_verify_function = s5kjn5_mot_do_factory_verify,
	...
	.enable_preload = 1,
	.preload_size = S5KJN5_MOT_EEPROM_DATA_SIZE,
};
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	int af_addr = S5KJN5_MOT_AF_ADDR;
	int af_size = S5KJN5_MOT_AF_DATA_SIZE;
	int awb_addr = S5KJN5_MOT_AWB_ADDR;
	int awb_size = S5KJN5_MOT_AWB_DATA_SIZE;
	...
	if(pCamCalData->sensorID == MOT_MERC_S5KJN5_SENSOR_ID) {
		//af posture calibration data
		unsigned char AF_POSTURE[S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE+2];
		unsigned int af_posture_data_offset = S5KJN5_MOT_MTK_NECESSARY_DATA_ADDR;
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				af_posture_data_offset, S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE+2, (unsigned char *) AF_POSTURE);
		if (read_data_size > 0)
			err = CAM_CAL_ERR_NO_ERR;
		else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
		...
		checkSum = AF_POSTURE[S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE]<<8 | AF_POSTURE[S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE+1];
		if(check_crc16(AF_POSTURE, S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
...
unsigned int mot_s5kjn5_do_pdaf(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
				(struct STRUCT_CAM_CAL_DATA_STRUCT *)pGetSensorCalData;
	...
	uint8_t *tempBuf = kmalloc(S5KJN5_MOT_PDAF_DATA_SIZE +4, GFP_KERNEL);
	if (!tempBuf)
	{
		return -ENOMEM;
	}

	pCamCalData->PDAF.Size_of_PDAF = S5KJN5_MOT_PDAF_DATA_SIZE;
	debug_log("PDAF start_addr =%x table_size=%d\n", S5KJN5_MOT_PDAF1_ADDR, S5KJN5_MOT_PDAF_DATA_SIZE);

	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			S5KJN5_MOT_PDAF1_ADDR, S5KJN5_MOT_PDAF_DATA_SIZE + 4, (unsigned char *)tempBuf);
	if (read_data_size <= 0) {
		err = CAM_CAL_ERR_NO_PDAF;
		return err;
	}
	checkSum1 = tempBuf[S5KJN5_MOT_PDAF_DATA_SIZE] << 8 | tempBuf[S5KJN5_MOT_PDAF_DATA_SIZE +1];
	checkSum2 = tempBuf[S5KJN5_MOT_PDAF_DATA_SIZE +2] << 8 | tempBuf[S5KJN5_MOT_PDAF_DATA_SIZE +3];
	debug_log("checkSum1  = 0x%x, checkSum2 = 0x%x", checkSum1, checkSum2);

	if(check_crc16(tempBuf, 496, checkSum1) && check_crc16(tempBuf +496, 1004+1100, checkSum2)) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			S5KJN5_MOT_PDAF1_ADDR, S5KJN5_MOT_PDAF_DATA_SIZE, (unsigned char *)&pCamCalData->PDAF.Data[0]);
	if (read_data_size <= 0) {
		err = CAM_CAL_ERR_NO_PDAF;
		return err;
	}
...
unsigned int s5kjn5_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData)
{
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			S5KJN5_MOT_EEPROM_ADDR, S5KJN5_MOT_EEPROM_DATA_SIZE, (unsigned char *)pCamCalData->DumpAllEepromData);
	if (read_data_size <= 0) {
		return CAM_CAL_ERR_NO_DEVICE;
	}
	//mnf check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_MNF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_MNF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData, S5KJN5_MOT_MNF_DATA_SIZE, checkSum)) {
	...
	memcpy(pCamCalData->serial_number, &pCamCalData->DumpAllEepromData[S5KJN5_MOT_SERIAL_NUMBER_ADDR],
		pCamCalData->serial_number_bit);

	//af check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_AF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_AF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_AF_ADDR, S5KJN5_MOT_AF_DATA_SIZE, checkSum)) {
	...
	//awb check
	if((mot_check_awb_data(pCamCalData->DumpAllEepromData+S5KJN5_MOT_AWB_ADDR, S5KJN5_MOT_AWB_DATA_SIZE + 2)) == NO_ERRORS) {
	...
	//lsc check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_LSC_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_LSC_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_LSC_ADDR, S5KJN5_MOT_LSC_DATA_SIZE, checkSum)) {
	...
	//pdaf check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_PDAF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_PDAF_CHECKSUM_ADDR+1]);
	checkSum1 = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_PDAF_CHECKSUM_ADDR+2])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_PDAF_CHECKSUM_ADDR+3]);
	debug_log("checkSum  = 0x%x, checkSum1 = 0x%x", checkSum, checkSum1);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_PDAF1_ADDR, S5KJN5_MOT_PDAF_DATA1_SIZE, checkSum)
		&& check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_PDAF2_ADDR, S5KJN5_MOT_PDAF_DATA2_SIZE, checkSum1)) {
	...
	//af sync data check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_AF_SYNC_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_AF_SYNC_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_AF_SYNC_DATA_ADDR, S5KJN5_MOT_AF_SYNC_DATA_SIZE, checkSum)) {
	...
	checkSum = (pCamCalData->DumpAllEepromData[S5KJN5_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJN5_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJN5_MOT_MTK_NECESSARY_DATA_ADDR, S5KJN5_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
