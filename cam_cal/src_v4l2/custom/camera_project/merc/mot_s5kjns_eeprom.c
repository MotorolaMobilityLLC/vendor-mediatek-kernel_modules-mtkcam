...
static unsigned int s5kjns_mot_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
...
#define S5KJNS_MOT_LSC_DATA_SIZE 0x074C
...
#define S5KJNS_MOT_PDAF_DATA_SIZE (S5KJNS_MOT_PDAF_DATA1_SIZE+S5KJNS_MOT_PDAF_DATA2_SIZE)
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x0000000B, 0x54516130, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000000, 0x00000000, 0x00000000, do_module_version},
		{0x00000000, 0x00000000, 0x00000025, do_part_number},
		{0x00000001, S5KJNS_MOT_LSC_ADDR, S5KJNS_MOT_LSC_DATA_SIZE, mot_do_single_lsc},
		{0x00000001, 0x00000027, 0x00000047, s5kjns_mot_do_2a_gain},
		{0x00000001, S5KJNS_MOT_PDAF1_ADDR, S5KJNS_MOT_PDAF_DATA_SIZE, mot_do_pdaf},
		...
		{0x00000001, S5KJNS_MOT_MNF_ADDR, S5KJNS_MOT_MNF_DATA_SIZE, mot_do_manufacture_info}
	}
};
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_s5kjns_eeprom = {
	...
	.sensor_id = MOT_MERC_S5KJNS_SENSOR_ID,
	.i2c_write_id = 0xA0,
	.max_size = 0x8000,
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	int af_addr = S5KJNS_MOT_AF_ADDR;
	int af_size = S5KJNS_MOT_AF_DATA_SIZE;
	int awb_addr = S5KJNS_MOT_AWB_ADDR;
	int awb_size = S5KJNS_MOT_AWB_DATA_SIZE;
	...
	if (0x2 & AWBAFConfig) {
		...
		pCamCalData->Single2A.S2aAF_t.AF_dac_code_bit_depth = 10;
	...
	if(pCamCalData->sensorID == MOT_MERC_S5KJNS_SENSOR_ID) {
		//af posture calibration data
		unsigned char AF_POSTURE[S5KJNS_MOT_MTK_NECESSARY_DATA_SIZE+2];
		unsigned int af_posture_data_offset = S5KJNS_MOT_MTK_NECESSARY_DATA_ADDR;
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				af_posture_data_offset, S5KJNS_MOT_MTK_NECESSARY_DATA_SIZE+2, (unsigned char *) AF_POSTURE);
		if (read_data_size > 0)
			err = CAM_CAL_ERR_NO_ERR;
		else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
		...
		checkSum = AF_POSTURE[S5KJNS_MOT_MTK_NECESSARY_DATA_SIZE]<<8 | AF_POSTURE[S5KJNS_MOT_MTK_NECESSARY_DATA_SIZE+1];
		if(check_crc16(AF_POSTURE, S5KJNS_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
