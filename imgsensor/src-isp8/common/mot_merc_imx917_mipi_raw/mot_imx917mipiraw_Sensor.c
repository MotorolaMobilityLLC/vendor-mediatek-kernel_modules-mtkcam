 ...
 *============================================================================
 ****************************************************************************/
#include "mot_imx917mipiraw_Sensor.h"
#include "mot_imx917_cali.h"
...
static int imx917_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx917_get_min_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx917_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int imx917_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int imx917_sensor_open(struct subdrv_ctx *ctx);
static int imx917_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
#if  ENABLE_IMX917_LONG_EXPOSURE
static int imx917_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
#endif
static int imx917_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, imx917_set_test_pattern},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, imx917_get_min_shutter},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, imx917_seamless_switch},
	{SENSOR_FEATURE_SET_AWB_GAIN, imx917_set_awb_gain},
#if  ENABLE_IMX917_LONG_EXPOSURE
	{SENSOR_FEATURE_SET_ESHUTTER, imx917_set_shutter},
#endif
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = imx917_preview_setting,
		.mode_setting_len = ARRAY_SIZE(imx917_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = imx917_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(imx917_seamless_preview),
		.hdr_mode = HDR_NONE,
		.pclk = 1360000000,
		.linelength = 11904,
		.framelength = 3791,
	...
	},
	{
		...
		.mode_setting_table = imx917_capture_setting,
		.mode_setting_len = ARRAY_SIZE(imx917_capture_setting),
	...
	},
	{
		...
		.mode_setting_table = imx917_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx917_normal_video_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = imx917_seamless_normal_video,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(imx917_seamless_normal_video),
		.hdr_mode = HDR_NONE,
		.pclk = 1360000000,
		.linelength = 11904,
		.framelength = 3791,
	...
	},
	{
		...
		.mode_setting_table = imx917_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx917_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = imx917_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx917_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{//custom1  full crop
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = imx917_seamless_custom1,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(imx917_seamless_custom1),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1360000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = imx917_ana_gain_table,
	.ana_gain_table_size = sizeof(imx917_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = imx917_init_setting,
	.init_setting_len = ARRAY_SIZE(imx917_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = imx917_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = imx917_sensor_open,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_MCLK, {24}, 0},
	{HW_ID_RST, {0}, 1000},
	{HW_ID_DOVDD, {1804000, 1804000}, 1000},
	{HW_ID_AVDD, {2804000, 2804000}, 1000}, //AVDD2_8V
	{HW_ID_AVDD2, {1800000, 1800000}, 1000},
	{HW_ID_DVDD, {1104000, 1104000}, 5000},
	{HW_ID_AFVDD, {2804000, 2804000}, 0},
	{HW_ID_MCLK_DRIVING_CURRENT, {4}, 1000},
	{HW_ID_RST, {1}, 5000}
};

const struct subdrv_entry mot_merc_imx917_mipi_raw_entry = {
	.name = "mot_merc_imx917_mipi_raw",
	.id = MOT_MERC_IMX917_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int imx917_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
static int imx917_get_min_shutter(struct subdrv_ctx *ctx, u8 *feature_para, u32 *feature_para_len)
{
	u64 *feature_data = (u64 *) feature_para;
	return imx917_get_min_shutter_by_scenario(ctx,
...
static int imx917_sensor_open(struct subdrv_ctx *ctx)
{
	...
	/* get sensor id */
	if (imx917_get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
...
static int imx917_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32* len)
{
	...
	imx917_set_multi_gain(ctx, values, exp_cnt);
...
static int imx917_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	imx917_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);

	return ERROR_NONE;
}

static int imx917_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
#if  ENABLE_IMX917_LONG_EXPOSURE
static void imx917_set_long_exposure(struct subdrv_ctx *ctx)
{
...
static void imx917_set_shutter_frame_length(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	int fine_integ_line = 0;
	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
...
static int imx917_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	imx917_set_shutter_frame_length(ctx, shutter,0);
	return 0;
}
#endif
...
static int imx917_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			imx917_set_multi_shutter_frame_length(ctx, (u64 *)&ae_ctrl->exposure, exp_cnt, 0);
			imx917_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			break;
