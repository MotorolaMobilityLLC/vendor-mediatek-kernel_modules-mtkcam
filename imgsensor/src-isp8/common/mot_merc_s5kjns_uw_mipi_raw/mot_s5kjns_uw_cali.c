...
static u8 s5kjns_uw_hw_ggc_data[S5KJNS_UW_HW_GGC_DATA_LEN+2];
static u16 hw_ggc_data[S5KJNS_UW_HW_GGC_DATA_LEN/2];
static u8 s5kjns_uw_hw_ggc_data_ready = 0;
#if S5KJNS_UW_HW_GGC_DEBUG
#ifdef DRV_LOG
#undef DRV_LOG
...
static int mot_s5kjns_uw_read_hw_ggc_data(struct subdrv_ctx *ctx)
{
	int ret = 0;
	u16 ref_crc = 0;
	if (s5kjns_uw_hw_ggc_data_ready) {
		DRV_LOG_MUST(ctx, "hw_ggc data is ready.");
		return 0;
	}

	ret = adaptor_i2c_rd_p8(ctx->i2c_client, (S5KJNS_UW_HW_GGC_EEPROM_ADDR >> 1), S5KJNS_UW_HW_GGC_DATA_START, s5kjns_uw_hw_ggc_data, S5KJNS_UW_HW_GGC_DATA_LEN+2) ;
	if (ret < 0) {
		DRV_LOGE(ctx, "Read HW_GGC data failed. ret:%d", ret);
		return -1;
	}

	ref_crc = ((s5kjns_uw_hw_ggc_data[S5KJNS_UW_HW_GGC_DATA_LEN] << 8) |s5kjns_uw_hw_ggc_data[S5KJNS_UW_HW_GGC_DATA_LEN+1]);
	if (s5kjns_uw_check_crc16(ctx, s5kjns_uw_hw_ggc_data, S5KJNS_UW_HW_GGC_DATA_LEN, ref_crc)) {
		s5kjns_uw_hw_ggc_data_ready = 1;
...
static void mot_s5kjns_uw_apply_hw_ggc_data(void *sensor_ctx)
{
	...
	if (s5kjns_uw_hw_ggc_data_ready) {
		...
		ret = adaptor_i2c_wr_u16(ctx->i2c_client, (ctx->i2c_write_id>>1),0x602A,0x0CFC);
 		for (k = 0; k < S5KJNS_UW_HW_GGC_DATA_LEN; k += 2)
 		{
 			hw_ggc_data[k/2] = (s5kjns_uw_hw_ggc_data[k] << 8) + s5kjns_uw_hw_ggc_data[k + 1];
#if S5KJNS_UW_HW_GGC_DEBUG
			DRV_LOG_MUST(ctx, "hw_ggc hw_ggc_data[%d]data is 0x%4x.",k/2,hw_ggc_data[k/2]);
#endif
			ret = adaptor_i2c_wr_u16(ctx->i2c_client, (ctx->i2c_write_id>>1),S5KJNS_UW_HW_GGC_REG_START,hw_ggc_data[k/2]);
			if (ret < 0) {
