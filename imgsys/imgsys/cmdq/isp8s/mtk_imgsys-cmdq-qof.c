...
bool is_qof_engine_enabled(int mod)
{
	...
	int addr = qof_reg_table[ISP8S_PWR_DIP][QOF_IMG_QOF_ENG_EN].addr;
...
static void mtk_qof_print_cg_status(void)
{
	...
	addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_DIP][QOF_IMG_QOF_STATE_DBG].addr);
	addr_val = readl(addr);
	...
	addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_TRAW][QOF_IMG_QOF_STATE_DBG].addr);
	addr_val = readl(addr);
	if (((addr_val & BIT(1)) == BIT(1)) ||
		((addr_val & BIT(6)) == BIT(6))) {
	...
	addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_QOF_STATE_DBG].addr);
	addr_val = readl(addr);
	if (((addr_val & BIT(1)) == BIT(1)) ||
		((addr_val & BIT(6)) == BIT(6))) {
	...
	addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_QOF_STATE_DBG].addr);
	addr_val = readl(addr);
	if (((addr_val & BIT(1)) == BIT(1)) ||
		((addr_val & BIT(6)) == BIT(6))) {
	...
	addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_3_LITE][QOF_IMG_QOF_STATE_DBG].addr);
	addr_val = readl(addr);
	if (((addr_val & BIT(1)) == BIT(1)) ||
		((addr_val & BIT(6)) == BIT(6))) {
		...
		struct cmdq_pkt *pkt)
{
	...
	imgsys_cmdq_restore_locked(imgsys_dev, pkt, &isp8s_module_data[ISP8S_PWR_WPE_2_TNR]);
	imgsys_cmdq_restore_locked(imgsys_dev, pkt, &isp8s_module_data[ISP8S_PWR_WPE_1_EIS]);
}
...
static void qof_start_pwr_loop(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	for (; (pwr < ISP8S_PWR_NUM); pwr++) {
		...
		if (pwr == ISP8S_PWR_WPE_2_TNR) // wa for gce thd lacked
			continue;
...
static void qof_set_engine_on(const u32 mod)
{
	...
	if (mod == ISP8S_PWR_WPE_1_EIS) { // wa for gce thd lacked
		qof_engine_on_setting(ISP8S_PWR_WPE_1_EIS);
		qof_engine_on_setting(ISP8S_PWR_WPE_2_TNR);

		io_addr =  QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_QOF_STATE_DBG].addr);
		if (readl_poll_timeout_atomic
			...
			mtk_imgsys_cmdq_qof_dump(0, false);
			g_qof_ver &= (~BIT(ISP8S_PWR_WPE_1_EIS));
			return;
		}
		io_addr =  QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_QOF_STATE_DBG].addr);
		if (readl_poll_timeout_atomic
			(io_addr, tmp, (tmp & BIT(1)) == BIT(1), POLL_DELAY_US, TIMEOUT_1000US) < 0) {
			QOF_LOGE("mod[%d] waiting for qof state pwr on timeout, disable support\n",ISP8S_PWR_WPE_2_TNR);
			mtk_imgsys_cmdq_qof_dump(0, false);
			g_qof_ver &= (~BIT(ISP8S_PWR_WPE_2_TNR));
			return;
		}

		/* qof APMCU Vote sub */
		io_addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_CLR].addr);
		write_mask(io_addr,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_CLR].val,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_CLR].mask);
		io_addr = QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_CLR].addr);
		write_mask(io_addr,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_CLR].val,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_CLR].mask);
...
static void qof_locked_set_engine_off(const u32 mod)
{
	...
	if (mod == ISP8S_PWR_WPE_1_EIS) { // wa for gce thd lacked
		io_addr =  QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_SET].addr);
		write_mask(io_addr,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_SET].val,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_APMCU_SET].mask);
		io_addr =  QOF_GET_REMAP_ADDR(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_SET].addr);
		write_mask(io_addr,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_SET].val,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_APMCU_SET].mask);
		...
		qof_engine_off_setting(ISP8S_PWR_WPE_1_EIS);
		qof_engine_off_setting(ISP8S_PWR_WPE_2_TNR);
	} else {
		io_addr =  QOF_GET_REMAP_ADDR(qof_reg_table[mod][QOF_IMG_APMCU_SET].addr);
		write_mask(io_addr,
			qof_reg_table[mod][QOF_IMG_APMCU_SET].val,
...
static void qof_module_vote_add(struct cmdq_pkt *pkt, u32 pwr, u32 user)
{
	...
	if (user == QOF_USER_AP) {
		...
		cmdq_pkt_write(pkt, NULL, (qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_ADD].addr) /*address*/,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_ADD].val,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_ADD].mask);
		cmdq_pkt_write(pkt, NULL, (qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_ADD].addr) /*address*/,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_ADD].val,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_ADD].mask);
		cmdq_pkt_poll_sleep(pkt, BIT(1)/*poll val*/,
			(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_QOF_STATE_DBG].addr), BIT(1) /*mask*/);
		cmdq_pkt_poll_sleep(pkt, BIT(1)/*poll val*/,
			(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_QOF_STATE_DBG].addr), BIT(1) /*mask*/);
...
static void qof_module_vote_sub(struct cmdq_pkt *pkt, u32 pwr, u32 user)
{
	...
	if (user == QOF_USER_AP) {
		...
		cmdq_pkt_write(pkt, NULL, (qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_SUB].addr) /*address*/,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_SUB].val,
			qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_EVENT_CNT_SUB].mask);
		cmdq_pkt_write(pkt, NULL, (qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_SUB].addr) /*address*/,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_SUB].val,
			qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_EVENT_CNT_SUB].mask);

		cmdq_pkt_poll_sleep(pkt, 0/*poll val*/,
			(qof_reg_table[ISP8S_PWR_WPE_1_EIS][QOF_IMG_QOF_STATE_DBG].addr)/*addr*/, BIT(3) /*mask*/);
		cmdq_pkt_poll_sleep(pkt, 0/*poll val*/,
			(qof_reg_table[ISP8S_PWR_WPE_2_TNR][QOF_IMG_QOF_STATE_DBG].addr)/*addr*/, BIT(3) /*mask*/);
...
void mtk_imgsys_cmdq_qof_add(struct cmdq_pkt *pkt, bool *qof_need_sub, u32 hw_comb)
{
	...
	for (pwr = ISP8S_PWR_START; pwr < ISP8S_PWR_NUM; pwr++) {
		if ((IS_MOD_SUPPORT_QOF(pwr)) &&
			...
			if (pwr == ISP8S_PWR_WPE_2_TNR)
				continue;
			if (pwr == ISP8S_PWR_WPE_1_EIS) // wa for gce thd lacked
